...
final class DirectCookiePool {
  ...
  private void load(){
    synchronized(lock){
      ...
      try{for(Map.Entry<String,?> value:prefs.getAll().entrySet())if(value.getKey().startsWith(ENTRY)&&value.getValue() instanceof String){Entry entry=entry(new JSONObject((String)value.getValue()));if(entry!=null)loaded.put(entry.id,entry);}}catch(Exception ignored){android.util.Log.w("DirectCookiePool.java", "DirectCookiePool.java Exception: "+ignored.getMessage(), ignored);}
      try{JSONArray legacy=new JSONArray(prefs.getString(LEGACY_ENTRIES,"[]"));for(int i=0;i<legacy.length();i++){Entry entry=entry(legacy.optJSONObject(i));if(entry!=null&&!loaded.containsKey(entry.id)){loaded.put(entry.id,entry);saveEntryLocked(entry);}}if(legacy.length()>0)prefs.edit().remove(LEGACY_ENTRIES).apply();}catch(Exception ignored){android.util.Log.w("DirectCookiePool.java", "DirectCookiePool.java Exception: "+ignored.getMessage(), ignored);}
  ...
  private void saveEntryLocked(Entry entry){try{prefs.edit().putString(ENTRY+entry.id,json(entry).toString()).apply();}catch(Exception ignored){android.util.Log.w("DirectCookiePool.java", "DirectCookiePool.java Exception: "+ignored.getMessage(), ignored);}}
