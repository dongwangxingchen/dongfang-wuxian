...
| P16 | 新增 `dfwx/RikkaHubEmbed.kt`（=上游 RouteActivity.AppRoutes 全量内容，差异仅：DI 改 koinInject、backStack 经回调上报、无障碍事件/音量键桥接为参数、无 SafeMode 检查）+ `dfwx/AiPageHost.kt`（宿主 Java 桥接入口，返回 ComposeView）；RouteActivity.AppRoutes 瘦身为对 RikkaHubEmbed 的 8 行调用（导航栈回调保留供分享/快捷入口 push） | 东方无限底栏"AI"=主界面内嵌页（ComposeView 进宿主 pageFrame），**不再跳转独立 Activity**——消除跳转延迟与"两个 App"割裂感（用户验收硬指标）；返回键经宿主 OnBackPressedDispatcher 桥接（宿主 MainActivity 已改 extends androidx.activity.ComponentActivity）；音量键滚动内嵌态暂不可用（已登记）。同步上游：对照上游 AppRoutes 增删重放 RikkaHubEmbed | 宿主交互定制；同步上游时需重放 |
| P17 | 新增 `ui/theme/presets/DongfangTheme.kt`（色板=宿主 ThemeEngine：BG#0B0A12/SURFACE#16141F/PRIMARY#A78BFA/TEXT#F2F0F7/MUTED#9A93AB/DIV#262332）+ PresetThemes 列表**置首**（全新安装默认主题）+ BuiltinProviderSeeder 播种时 themeId="dfwx" | AI 界面配色/质感与东方无限一体（用户验收硬指标）；用户仍可在 设置→主题 更换 | 品牌定制；同步上游时需重放（新增文件+列表置首+seeder 一行） |
| P18 | 4 文件手表小屏适配（EmbedShotsJvmTest 343dp 截图验收 2026-09-19 发现）：`ui/pages/extensions/QuickMessagesPage.kt`/`skills/SkillsPage.kt`/`workspace/WorkspacePage.kt` 空状态提示 Text 加 `padding(horizontal=72.dp)`（文字曾延伸进右下 FAB 被截断）；`ui/pages/setting/SettingWebPage.kt` LazyColumn contentPadding 底部 8→88dp（Start 按钮曾压住 localhost 卡片） | 手表尺寸下空状态文案可完整阅读、按钮不再遮挡内容 | 宿主体验定制；同步上游时需重放（四处各一行 modifier/contentPadding） |
